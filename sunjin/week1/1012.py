...
def BFS(x,y):           
    queue = [(x,y)] # 시작 위치 (x, y)를 받아와서 그 위치를 큐에 추가한다. -> dequeue를 사용하면 popleft -> queue.pop할 때 O(N) 시간복잡도가 발생하지 않을 것이다?
    matrix[x][y] = 0 # 해당 위치의 값을 0으로 바꾸어 방문한 것으로 표시한다.

    while queue: # 큐가 비어있을 때까지 반복문을 실행한다.
        x, y = queue.pop(0) # 큐에서 하나의 위치를 꺼낸다. -> O(N)

        for i in range(4):
            nx = x + dx[i]
            ny = y + dy[i]

            if nx < 0 or nx >= M or ny < 0 or ny >= N: # 현재 위치에서 상, 하, 좌, 우 방향으로 이동하면서 유효한 위치인지 확인한다.
